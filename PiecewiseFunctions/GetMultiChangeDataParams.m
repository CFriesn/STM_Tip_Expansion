function par = GetMultiChangeDataParams()
% Assumed units:
% Power: mW
% Temperature: K
% Length: mm --> Vol: mm^3
...
par.Model.T_0 = 60; %Initial Tip and holder temperature
par.Model.OptimizeVars = [true false false true]; %[k LHolder LTip LaserTrans] Set which parameters to optimize 
par.Model.VarUpperBounds = [inf  20 5 1];
par.Model.VarLowerBounds = [0 10 1 0.05];
par.Model.VarInitial = [0.20 15 1.01 0.35]; %[k LHolder LTip LaserTrans] Initial values for the optimizer. If var not optimized, this will be taken as the fixed value.
...
par.Model.Holder.V = 2*pi*par.Model.Holder.Radius*par.Model.VarInitial(2);
...
par.Model.Tip.V = 2*pi*par.Model.Tip.Radius*par.Model.VarInitial(3);
